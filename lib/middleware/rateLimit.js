...
function createRateLimiter({ max, windowMs, key = 'default' }) {
  return {
    ...
    async consume(ip) {
      // Loopback exemption is for local/CI only — Vercel always sets a real client
      // IP in prod, but skipping the check there too would let anyone bypass the
      // limit by spoofing x-forwarded-for to a loopback address.
      const loopbackExempt = process.env.NODE_ENV !== 'production' && ip && LOOPBACK_IPS.has(ip);
      if (!ip || loopbackExempt) return { allowed: true };
