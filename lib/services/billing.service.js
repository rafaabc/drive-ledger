...
async function handleWebhookEvent(event) {
  ...
  // An admin-granted comp (planSource:'manual') must not be silently
  // overwritten by a Stripe event for the same customer — the admin's
  // grant stays authoritative until explicitly released.
  if (user.planSource === 'manual') return;

  // Guard against ANY previously-processed event, not just the immediately-prior
  // one: Stripe does not guarantee delivery ordering and retries for up to 3 days,
  // so a stale event can be redelivered after a later event has already changed
  // state. A single lastStripeEventId field would miss that case.
  if (user.processedStripeEventIds?.includes(event.id)) return; // already processed — idempotent no-op

  switch (event.type) {
    case 'checkout.session.completed': {
      await userModel.updatePlanAndBilling(
        user._id,
        {
          plan: 'pro',
          planSource: 'stripe',
    ...
    case 'customer.subscription.deleted': {
      ...
      // clearBilling() does NOT touch processedStripeEventIds — that history must
      // survive cancellation, otherwise a stale pre-cancellation event redelivered
      // afterward would no longer be recognized as already-processed.
