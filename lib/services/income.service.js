...
function computeFuelWindowStats(vehicleExpenses, periodStart, periodEnd) {
  ...
  const hasUnloggedFill = vehicleExpenses.some((e) => {
    if (e.category !== 'Fuel' || typeof e.odometer === 'number') return false;
    const d = new Date(e.date);
    return d >= spanStart && d <= spanEnd;
  });
  if (hasUnloggedFill) result.flags = [...result.flags, 'missingOdometer'];
 ...
 */
function buildMonthlyBreakdown(income, vehicleExpenses, year) {
  const months = [];
  for (let m = 1; m <= 12; m++) {
    ...
    const shiftTotals = computeShiftTotals(monthIncome, totalIncome, costPerKm);
...
async function getProfitSummary(userId, query) {
  ...
  if (breakdown === 'monthly' && !month) {
    result.months = buildMonthlyBreakdown(income, vehicleExpenses, year);
  }
