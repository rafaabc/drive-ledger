...
// User-controlled free text (e.g. reminder.title) is interpolated straight into these
// HTML email templates — escape it so a reminder title can't inject markup/links.
...
async function sendAccountExistsEmail({ to }) {
  if (!process.env.RESEND_API_KEY) return;
  const loginUrl = `${process.env.FRONTEND_URL || process.env.BASE_URL}/login`;
  const forgotUrl = `${process.env.FRONTEND_URL || process.env.BASE_URL}/forgot-password`;
  await getResend().emails.send({
    from: fromAddress(),
    to,
    subject: 'Você já tem uma conta — Norevify',
    html: `
      <p>Alguém tentou criar uma conta no <strong>Norevify</strong> usando este email, mas você já tem uma conta.</p>
      <p><a href="${loginUrl}">Fazer login</a> ou, se esqueceu sua senha, <a href="${forgotUrl}">redefinir sua senha</a>.</p>
      <p>Se não foi você, ignore este email — sua conta continua segura.</p>
    `,
  });
}

function formatReminderLine(r) {
  const parts = [escapeHtml(r.type)];
