...
// title is free text stored as-is and later interpolated into the reminder-digest
// HTML email (see email.service.js) — bound its type/length here so a malformed
// value can't reach that template in the first place.
function validateTitle(title) {
  if (title === undefined) return;
  if (typeof title !== 'string') throw makeError(400, 'title must be a string');
  if (title.length > TITLE_MAX_LENGTH)
    throw makeError(400, `title must be at most ${TITLE_MAX_LENGTH} characters`);
}
...
function validateCreateBody(body) {
  ...
  if (body.dueDate !== undefined) {
    ...
    // One full day of tolerance absorbs client/server timezone offsets (server runs in
    // UTC; a client west of UTC serializes "today"/"tomorrow" as an earlier UTC instant),
    // so a future local date is never rejected as past.
    if (d.getTime() < startOfTodayUtc - DAY_MS)
      throw makeError(400, 'dueDate cannot be in the past');
  }
...
async function currentKmByVehicle(userId) {
  const vehicles = await vehicleModel.findByUserId(userId);
  const map = new Map();
  for (const v of vehicles) map.set(v.id, v.currentKm || 0);
  return map;
}

async function listReminders(userId, query) {
  const kmByVehicle = await currentKmByVehicle(userId);
  let all = await reminderModel.findByUserId(userId);
  if (query.vehicleId) all = all.filter((r) => r.vehicleId?.toString() === query.vehicleId);
  const decorated = all.map((r) => serialize(r, kmByVehicle.get(r.vehicleId?.toString()) || 0));
...
async function getReminder(userId, id) {
  ...
  const kmByVehicle = await currentKmByVehicle(userId);
  return serialize(r, kmByVehicle.get(r.vehicleId?.toString()) || 0);
}
...
async function updateReminder(userId, id, body) {
  ...
  // Moving the due target invalidates the last notified status — otherwise
  // editing dueDate/dueKm back into the dueSoon/overdue zone would silently
  // skip re-notifying since the status string would match the stale marker.
  if (body.dueDate !== undefined || body.dueKm !== undefined) {
    merged.lastNotifiedStatus = null;
  }
...
async function getBadgeCount(userId) {
  ...
  let dueSoon = 0,
    overdue = 0;
  for (const r of all) {
    const s = computeStatus(r, kmByVehicle.get(r.vehicleId?.toString()) || 0);
